TAG = "OF-Temperature"
OF_LOGI(TAG, "Call Temperature lua script!")
    ...
    fs = [[
        ...
        void main()
        {
			...
			if (uIntensity < 0.0){
				...
				vec4 newColor1 = texture2D(uTexture2, texPos1);
...
function Filter:onApplyParams(context, filter)
    ...
    local path = filter:resParam("ImageMin")
	local fullpath = filter:resFullPath(path)
	if self.imageTexMin then context:destroyTexture(self.imageTexMin) end -- destroy if exist
	self.imageTexMin = context:loadTextureFromFile(fullpath, TEXTURE_2D, LINEAR, CLAMP_TO_EDGE, false, false)

	path = filter:resParam("ImageMax")
	fullpath = filter:resFullPath(path)
...
function Filter:applyRGBA(context, filter, frameData, inTex, outTex, debugTex)
	...
	self.renderPass:setUniformTexture("uTexture1", 1, self.imageTexMin:textureID(), TEXTURE_2D)
	self.renderPass:setUniformTexture("uTexture2", 2, self.imageTexMax:textureID(), TEXTURE_2D)
