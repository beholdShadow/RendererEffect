...
function EffectList:initParams(context, filter)
	...
	filter:insertIntParam("Brightness", -100, 100, 0)
	filter:insertIntParam("Contrast", -100, 100, 0)
	filter:insertIntParam("Saturation", -100, 100, 0)
	...
	filter:insertIntParam("Temperature", -100, 100, 0)
	filter:insertIntParam("Tone", -100, 100, 0)
...
function EffectList:onApplyParams(context, filter)
	...
	self:setEnabled(2,  self.params.thinface > 0)
	self:setEnabled(3,  self.params.brightness  ~= 0)
    self:setEnabled(4,  self.params.contrast    ~= 0)
	self:setEnabled(5, self.params.saturation  ~= 0)
	self:setEnabled(6,  self.params.sharpen     ~= 0)
	self:setEnabled(7,  self.params.highlight   ~= 0)
	self:setEnabled(8,  self.params.shadow      ~= 0)
	self:setEnabled(9,  self.params.fade        ~= 0)
	self:setEnabled(10, self.params.temperature ~= 0)
	self:setEnabled(11,  self.params.tone        ~= 0)

	self:sendMessage(context, 1,   Json.TableToJson({ beauty = self.params.beauty }))
	self:sendMessage(context, 2,   Json.TableToJson({ thinface = self.params.thinface }))
	self:sendMessage(context, 3,   Json.TableToJson({ intensity = self.params.brightness / 100 }))
	self:sendMessage(context, 4,   Json.TableToJson({ intensity = self.params.contrast / 100 }))
	self:sendMessage(context, 5,  Json.TableToJson({ intensity = self.params.saturation / 100 * 2.3 }))
	self:sendMessage(context, 6,   Json.TableToJson({ intensity = self.params.sharpen / 100 }))
	self:sendMessage(context, 7,   Json.TableToJson({ intensity = self.params.highlight / 100 }))
	self:sendMessage(context, 8,   Json.TableToJson({ intensity = self.params.shadow / 100 }))
	self:sendMessage(context, 9,   Json.TableToJson({ intensity = self.params.fade / 100 }))
	self:sendMessage(context, 10,  Json.TableToJson({ intensity = self.params.temperature / 100 }))
	self:sendMessage(context, 11,   Json.TableToJson({ intensity = self.params.tone / 100 }))
end
