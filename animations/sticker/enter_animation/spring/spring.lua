...
function Spring:apply(filter)
    ...
    local scale = 1.0 * self.smoothstep(self, 0, perStepDuration, self.timestamp) -
                    0.5 * self.smoothstep(self, perStepDuration, 2 * perStepDuration, self.timestamp) +
                    ...
                    0.02 * self.smoothstep(self, 4 * perStepDuration, 5 * perStepDuration, self.timestamp) -
                    0.02 * self.smoothstep(self, 5 * perStepDuration, 6 * perStepDuration, self.timestamp)

    filter.animation.params.localScale[1] = 0.5 + scale
    filter.animation.params.localScale[2] = 0.5 + scale
    filter.animation.params.localScale[3] = 0.5 + scale
end
