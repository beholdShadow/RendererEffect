...
function WaveSliderOut:init(context)
    ...
    self.params = {
        ...
        },
        {
            ...
            keycurves = {
                ...
                },
                {
                    ...
                    endTime = 0.45 -- jieshushijian
                },
                {
                    obj = self.values,
                    startValue = 0.0, -- qishizhi
                    endValue = 0.45, -- jieshuzhi
                    defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
                    actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
                        renderPass:setUniform1f(key, value)
                    end,
                    curve = function(t, b, c, d)
                        t = t / d
                        local controls = {0.91, 0.0, 0.3, 0.9} -- beisaierquxiancanshu
                        local tvalue = getBezierTfromX(controls, t)
                        local value = getBezierValue(controls, tvalue)
                        return b + c * value[2]
                    end,
                    startTime = 0.65, -- qishishijian
                    endTime = 1.0 -- jieshushijian
                }
            }
        },
        {
            -- tupianfangdadezhi，congdadaoxiao，julie
            key = "enLarge",
            keycurves = {
                {
                    obj = self.values,
                    startValue = 4.0, -- qishizhi
                    endValue = 1.2, -- jieshuzhi
                    ...
                    curve = function(t, b, c, d)
                        t = t / d
                        local controls = {0.11, 0.76, 0.33, 1} -- beisaierquxiancanshu
                        local tvalue = getBezierTfromX(controls, t)
                        local value = getBezierValue(controls, tvalue)
                        return b + c * value[2]
                    end,
                ...
                },
                {
                    ...
                    curve = function(t, b, c, d)
                        t = t / d
                        local controls = {0.38, 0.5, 0.39, 0.92} -- beisaierquxiancanshu
                        local tvalue = getBezierTfromX(controls, t)
                        local value = getBezierValue(controls, tvalue)
                        return b + c * value[2]
                    end,
                    startTime = 0.1, -- qishishijian
                    endTime = 0.9-- jieshushijian
                }
            }
        },
        {
            -- suofangmohuqiangdu，zhujianjianruo
            key = "scaleBlurStep",
            obj = self.values,
            ...
            curve = function(t, b, c, d)
                ...
                local controls = {.18, .67, .55, .91} -- beisaierquxiancanshu
                local tvalue = getBezierTfromX(controls, t)
                local value = getBezierValue(controls, tvalue)
                return b + c * value[2]
            end,
        ...
        },
        {
            ...
            key = "transform",
            obj = self.values,
            startValue = Vec2f.new(0, 0), -- qishizhi
            endValue = Vec2f.new(1, -2), -- jieshuzhi
            ...
            curve = function(t, b, c, d)
                t = t / d
                local controls = {0.62, 0.00, 0.93, 0.17} -- beisaierquxiancanshu
                local tvalue = getBezierTfromX(controls, t)
                local value = getBezierValue(controls, tvalue)
                return b + c * value[2]
            end,
            startTime = 0.8, -- qishishijian
            endTime = 1.0 -- jieshushijian
        },
        {
            ...
            curve = function(t, b, c, d)
                t = t / d
                local controls = {1, 1, 1, 1} -- beisaierquxiancanshu
...
function WaveSliderOut:clear(context)
...
function WaveSliderOut:setDuration(filter, duration)
    self.duration = duration
end

function WaveSliderOut:seek(filter, timestamp)
    self.timestamp = timestamp
end

function WaveSliderOut:apply(filter)
    filter.animation.params.renderToRT = true
end

function WaveSliderOut:AnimateParam(progress)
    for i = 1, #self.params do
        ...
        if param.key ~= "transform" then
            OF_LOGI("WaveSliderOut", string.format("WaveSliderOut:AnimateParam key = %s, value = %f", param.key, self.values[param.key]))
        end
    end
end

function WaveSliderOut:applyEffect(filter, outTex)
    ...
    self.renderPass:setUniform1f("distortSpeed", 4.0)
    self.renderPass:setUniform1f("xAddOffset", 0.0)
    self.renderPass:setUniform1f("yAddOffset", 0.0)
    self.renderPass:setUniform1f("xDistMag", -0.07)
    self.renderPass:setUniform1f("yDistMag", -0.07)
