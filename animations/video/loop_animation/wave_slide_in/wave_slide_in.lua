...
function WaveSlideIn:init(context)
        ...
        },
        {
            ...
            keycurves = {
                ...
                },
                {
                    ...
                    curve = function(t, b, c, d)
                        t = t / d
                        local controls = {0.77, 0.02, 0.96, 0.27} -- beisaierquxiancanshu
                        local tvalue = getBezierTfromX(controls, t)
                        local value = getBezierValue(controls, tvalue)
                        return b + c * value[2]
                    end,
                    startTime = 0.85, -- qishishijian
                    endTime = 1.0
                }
            }
         -- jieshushijian
...
function WaveSlideIn:clear(context)
...
function WaveSlideIn:setDuration(filter, duration)
    self.duration = duration
end

function WaveSlideIn:seek(filter, timestamp)
    self.timestamp = timestamp
end

function WaveSlideIn:apply(filter)
    filter.animation.params.renderToRT = true
end

function WaveSlideIn:AnimateParam(progress)
    for i = 1, #self.params do
        ...
        if param.key ~= "transform" then
            OF_LOGI("WaveSlideIn", string.format("WaveSlideIn:AnimateParam key = %s, value = %f", param.key, self.values[param.key]))
        end
    end
end

function WaveSlideIn:applyEffect(filter, outTex)
