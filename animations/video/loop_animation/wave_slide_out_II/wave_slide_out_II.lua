...
function WaveSliderOutII:init(context)
...
function WaveSliderOutII:clear(context)
...
function WaveSliderOutII:setDuration(filter, duration)
    self.duration = duration
end

function WaveSliderOutII:seek(filter, timestamp)
    self.timestamp = timestamp
end

function WaveSliderOutII:apply(filter)
    filter.animation.params.renderToRT = true
end

function WaveSliderOutII:AnimateParam(progress)
    for i = 1, #self.params do
        ...
        if param.key ~= "transform" then
            OF_LOGI("WaveSliderOutII", string.format("WaveSliderOutII:AnimateParam key = %s, value = %f", param.key, self.values[param.key]))
        end
    end
end

function WaveSliderOutII:applyEffect(filter, outTex)
    ...
    self.renderPass:setUniform1i("USE_DIR_BLUR", 0)
    self.renderPass:setUniform2f("transform", 0.0, 0.0)
