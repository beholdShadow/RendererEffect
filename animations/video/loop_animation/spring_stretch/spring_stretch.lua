local SpringStretch = {
    ...
    fs = [[
        ...
        void main()
        {
            vec2 fragCoord = mirrorRepeatUV(TexCoords);
            ...
            if (USE_DIR_BLUR > 0) {
                // motionBlur
                vec4 dirBlur = getDirectionBlur(u_inputTexture,fragCoord,blurDirection);
...
local function funcEaseAction1(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.215, 0.61, 0.355, 1}
...
local function funcEaseAction2(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.25, 0.46, 0.45, 0.94}
    local tvalue = getBezierTfromX(controls, t)
    local value =  getBezierValue(controls, tvalue)
    return b + c * value[2]
end
...
local function funcEaseAction4(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.55, 0.055, 0.675, 0.19}
    local tvalue = getBezierTfromX(controls, t)
    local value =  getBezierValue(controls, tvalue)
    return b + c * value[2]
end

local function funcEaseBlurAction4(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.55, 0.055, 0.675, 0.19}
    local tvalue = getBezierTfromX(controls, t)
...
local function funcEaseAction5(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.55, 0.085, 0.68, 0.53}
    local tvalue = getBezierTfromX(controls, t)
    local value =  getBezierValue(controls, tvalue)
    return b + c * value[2]
end

local function funcEaseBlurAction5(t, b, c, d)
    t = t/d
    -- 第一阶段的位移曲线，贝塞尔曲线版本
    local controls = {0.55, 0.085, 0.68, 0.53}
    local tvalue = getBezierTfromX(controls, t)
...
local function funcEaseAction3(t, b, c, d)
    ...
    if t~=0.0 and t~=1.0 then
        t = 2 ^ (-10 * (t/ 2.0)) * math.sin((t/2.0 - 0.4 / 4) * (2 *math.pi) / 0.4) + 1
    end
    return b + c * t
end

function SpringStretch:init(context)
    ...
    self.params = {
        {
            ...
            keycurves = {
                {
                    obj = self.values,
                    startValue = Vec3f.new(1.5, 1.5, 0.0), -- qishizhi
                ...
                {
                    obj = self.values,
                    startValue = Vec3f.new(1.0, 1.0, 0.0), -- qishizhi
                    endValue = Vec3f.new(1.3, 1.3, 1.0), -- jieshuzhi
                    defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
                    actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
                    end,
                ...
                {
                    obj = self.values,
                    startValue = Vec3f.new(1.3, 1.3, 0.0), -- qishizhi
                    endValue = Vec3f.new(0.8, 0.8, 1.0), -- jieshuzhi
                    defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
                    actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
                    end,
                    curve = funcEaseAction4,
        ...
        },
        {
            ...
            keycurves = {
                {
                    obj = self.values,
                    startValue = Vec3f.new(0.0, 0.0, 5.0), -- qishizhi
                ...
                },
                {
                    ...
                    endValue = Vec3f.new(0.0, 0.0, -5.0), -- jieshuzhi
                    defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
                    actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
                    end,
                    curve = funcEaseAction2,
        ...
        },
        {
            ...
            key = "localPosition",
            obj = self.values,
            startValue = Vec3f.new(0.0, 0.1, 0.0), -- qishizhi
            endValue = Vec3f.new(0.0, 0.0, 0.0), -- jieshuzhi
            defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
            actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
            end,
...
function SpringStretch:clear(context)
...
function SpringStretch:setDuration(filter, duration)
    self.duration = duration
end

function SpringStretch:seek(filter, timestamp)
    self.timestamp = timestamp
end

function SpringStretch:apply(filter)
    filter.animation.params.renderToRT = true
end

function SpringStretch:AnimateParam(progress)
...
function SpringStretch:applyEffect(filter, outTex)
