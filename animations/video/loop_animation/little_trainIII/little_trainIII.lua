...
local function funcEaseAction3(t, b, c, d)
    t = t/d
    -- diyijieduandeweiyiquxian，beisaierquxianbanben
    local controls = {.5,0,.75,0}
    local tvalue = getBezierTfromX(controls, t)
    local value =  getBezierValue(controls, tvalue)
    return b + c * value[2]
end

function LittleTrainIII:init(context)
    ...
    self.params = {
        ...
        },
        {
            ...
            keycurves = {
                {
                    obj = self.values,
                    startValue = Vec3f.new(2.0, 2.0, 1.0), -- qishizhi
...
function LittleTrainIII:clear(context)
...
function LittleTrainIII:setDuration(filter, duration)
    self.duration = duration
end

function LittleTrainIII:seek(filter, timestamp)
    self.timestamp = timestamp
end

function LittleTrainIII:apply(filter)
    filter.animation.params.renderToRT = true
end

function LittleTrainIII:AnimateParam(progress)
...
function LittleTrainIII:applyEffect(filter, outTex)
