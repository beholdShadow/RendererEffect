...
local function funcEaseBlurAction4(t, b, c, d)
    t=t/d
    -- 第二阶段的模糊曲线，要和上面的位移曲线一致
    local controls = {.94,.01,.89,.15}
    local tvalue = getBezierTfromX(controls, t)
    local deriva = getBezierDerivative(controls, tvalue)
    return b + math.abs(deriva[2] / deriva[1]) * c
end

function JitterInII:init(context)
    ...
    self.params = {
        {
            ...
            keycurves = {
                {
                    obj = self.values,
                    startValue = Vec3f.new(1.8, 1.8, 0.0), -- qishizhi
                ...
                },
                {
                    ...
                    endValue = Vec3f.new(2.0, 2.0, 1.0), -- jieshuzhi
                    defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
                    actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
                    end,
        ...
        },
        {
            ...
            keycurves = {
                {
                    obj = self.values,
                    startValue = Vec3f.new(0.0, 0.0, -20.0), -- qishizhi
        ...
        },
        {
            ...
            key = "localPosition",
            obj = self.values,
            startValue = Vec3f.new(1.0, 0.0, 0.0), -- qishizhi
            endValue = Vec3f.new(0.0, 0.0, 0.0), -- jieshuzhi
            defaultValue = 0.0, -- morenzhi，gaijieduanmeiyouzhixing，huozhezhixingwanchengzhihougaiyingyongdezhi
            actionHandle = function(renderPass, key, value) -- seekshidehuidiaofangfa
            end,
...
function JitterInII:clear(context)
...
function JitterInII:setDuration(filter, duration)
    self.duration = duration
end

function JitterInII:seek(filter, timestamp)
    self.timestamp = timestamp
end

function JitterInII:apply(filter)
    filter.animation.params.renderToRT = true
end

function JitterInII:AnimateParam(progress)
...
function JitterInII:applyEffect(filter, outTex)
