...
function LeftRotateUpwards:init(context)
    self.renderPass = context:createCustomShaderPass(self.vs, self.fs)
end

function LeftRotateUpwards:clear(context)
...
function LeftRotateUpwards:setDuration(filter, duration)
    self.duration = duration
end

function LeftRotateUpwards:seek(filter, timestamp)
    self.timestamp = timestamp
end

function LeftRotateUpwards:apply(filter)
    filter.animation.params.renderToRT = true
end

function LeftRotateUpwards:applyEffect(filter, outTex)
    ...
    local blurScale = (self.timestamp / self.duration - 1.0)^2
    local radian = (self.timestamp / self.duration - 1.0) * math.pi / 2
    ...
    -- self.renderPass:setUniform1f("uTexWidth", filter.imageTex:width())
    -- self.renderPass:setUniform1f("uTexHeight", filter.imageTex:height())
