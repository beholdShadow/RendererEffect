...
function SwirlRotateIn:init(context)
    self.renderPass = context:createCustomShaderPass(self.vs, self.fs)
end

function SwirlRotateIn:clear(context)
...
function SwirlRotateIn:setDuration(filter, duration)
    self.duration = duration
end

function SwirlRotateIn:seek(filter, timestamp)
    self.timestamp = timestamp
end

function SwirlRotateIn:apply(filter)
    filter.animation.params.renderToRT = true
end

function SwirlRotateIn:applyEffect(filter, outTex)
    ...
    local radius = (1.0 - ratio) * math.min(filter.imageTex:width() * filter.params.scale, filter.imageTex:height() * filter.params.scale) / height
    local angle = 2.0
