...
function LeftRotate:init(context)
    self.renderPass = context:createCustomShaderPass(self.vs, self.fs)
end

function LeftRotate:clear(context)
...
function LeftRotate:setDuration(filter, duration)
    self.duration = duration
end

function LeftRotate:seek(filter, timestamp)
    self.timestamp = timestamp
end

function LeftRotate:apply(filter)
    filter.animation.params.renderToRT = true
end

function LeftRotate:applyEffect(filter, outTex)
    ...
    local radian = -(ratio - 1.0) * math.pi / 2   
