...
function Sway:init(filter)
end

function Sway:clear(filter)
end

function Sway:setDuration(filter, duration)
    self.duration = duration
end

function Sway:seek(filter, timestamp)
    self.timestamp = timestamp
end

function Sway:clamp(x, min, max)
    return math.max(math.min(x, max), min)
end

function Sway:smoothstep(edge0, edge1, x) 
    x = self.clamp(self, (x - edge0) / (edge1 - edge0), 0.0, 1.0)
    return x * x * (3 - 2 * x)
end

function Sway:apply(filter, outTex) 
    ...
    local rotateRad = math.pi / 12 * math.sin(4*math.pi / self.duration * self.timestamp)
    ...
    local center = {
        -- x = 8 * (self.smoothstep(self, 0.5, 0.75, ratio) - self.smoothstep(self, 0.75, 1.0, ratio)) - 4,
        -- y = 4 * math.sin(2*math.pi / self.duration * (self.timestamp - self.duration / 4)) + 4
        y = math.sin(2*math.pi / self.duration * self.timestamp) * 10 + 10,
        x = math.cos(2*math.pi / self.duration * self.timestamp) * 10
    } 
    ...
    local mvpMat = Matrix4f:TransMat(lineInfo.maxLineWidth / 2.0, -(lineInfo.totalHeight / 2  - lineInfo.maxLineHeight), 0.0) *
                    rotMat * Matrix4f:TransMat(center.x, -center.y, 0) *
...
function Sway:applyEffect(label, srcTex, dstTex)
