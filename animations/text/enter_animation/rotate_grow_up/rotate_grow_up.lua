...
function RotateGrowUp:apply(filter, outTex) 
    ...
    local lastDuration = (2 * self.duration) / (0.1 * n * n + n + 0.4)

    local curCharTimestamp = 0
    for i = 1, #filter.label.charsBackup do
        local char = filter.label.chars[i]
        local charBackup = filter.label.charsBackup[i]
        
        local perCharDuration = lastDuration * 0.2 * (n - i) + lastDuration;
            
        local progress = self.smoothstep(self, curCharTimestamp, curCharTimestamp + perCharDuration, self.timestamp) 

        local extraRot1 = self.smoothstep(self, curCharTimestamp, curCharTimestamp + 0.9 * perCharDuration, self.timestamp) 

        local extraRot2 = self.squarestep(self, curCharTimestamp + 0.9 * perCharDuration, curCharTimestamp + perCharDuration, self.timestamp) 

        local scaleMat = Matrix4f:ScaleMat(progress, progress, 1.0)

        curCharTimestamp = curCharTimestamp + perCharDuration * 0.15;

        local rotData = 5 * (n - i + 1) / n + 5
        -- local rot = (1.0 - extraRot1) * (90 + extraRot) - extraRot + extraRot2 * extraRot
        local rot = (1.0 - extraRot1) * (90 + rotData) - rotData + extraRot2 * rotData
        local rotMat = Matrix4f:RotMat(0.0, 0.0, rot / 180 * math.pi)
        ...
        for k = 1, 4 do
            local pos = Vec4f.new(charBackup.pos[2*k-1], charBackup.pos[2*k], 0.0, 1.0)
            pos = mvpMat * pos 
            char.pos[2*k-1] = pos.x
            char.pos[2*k] = pos.y
        end
    end
end
