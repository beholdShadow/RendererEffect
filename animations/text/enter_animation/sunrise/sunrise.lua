local Sunrise = {
    ...
    fs = [[
        ...
        void main()
        {
            ...
            float R = 1.0 - uRatio;
            if (vTexCoord.y >= R)
                resColor = texture2D(uTexture0, vec2(vTexCoord.x, vTexCoord.y - R));
            else
...
function Sunrise:setDuration(filter, duration)
    self.duration = duration
end

function Sunrise:seek(filter, timestamp)
    self.timestamp = timestamp
end

function Sunrise:apply(filter, outTex) 
end

function Sunrise:applyEffect(label, srcTex, dstTex)
