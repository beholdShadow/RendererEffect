...
function RotateLeftIn:init(filter)
end

function RotateLeftIn:clear(filter)
end

function RotateLeftIn:setDuration(filter, duration)
    self.duration = duration
end

function RotateLeftIn:seek(filter, timestamp)
    self.timestamp = timestamp
end

function RotateLeftIn:linearstep(edge0, edge1, x) 
    return math.min(1.0, math.max(0.0, (x - edge0) / (edge1 - edge0)))
end

function RotateLeftIn:apply(filter, outTex) 
    ...
    -- OF_LOGI(self.tag, string.format("RotateLeftIn:applyEffect filter.style.color1.w =%f alpha = %f", filter.style.color1.w,alpha))
...
function RotateLeftIn:applyEffect(label, srcTex, dstTex)
