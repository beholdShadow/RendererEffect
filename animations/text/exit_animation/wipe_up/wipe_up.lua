local WipeUp = {
    ...
    fs = [[
        ...
        {
            float w = 0.5;
            float alpha = clamp(-1.0 / w * vTexCoord.y + (1.0 + w) / w + uRatio * (-(1.0 + w) / w), 0.0, 1.0);

            vec4 color2 = texture2D(uTexture0, vTexCoord);

            //gl_FragColor = vec4(color2.rgb, mix(0.0, color2.a, alpha));
...
function WipeUp:setDuration(filter, duration)
    self.duration = duration
end

function WipeUp:seek(filter, timestamp)
    self.timestamp = timestamp
end

function WipeUp:apply(filter, outTex) 
end

function WipeUp:applyEffect(label, srcTex, dstTex)
