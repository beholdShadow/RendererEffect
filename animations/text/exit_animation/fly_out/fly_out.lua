...
function FlyOut:init(filter)
end

function FlyOut:clear(filter)
end

function FlyOut:animate()
    return {
        -- ['playSpeed'] = 30,
        ["anchor"] = {0, 0},
        ["pivot"] = {0, 0},
...
function FlyOut:animateChar(char)
    return {
        ["mode"] = 0,
        ["duration"] = .1,
        ["anchor"] = {0, 0},
...
function FlyOut:setDuration(filter, duration)
    self.duration = duration
end

function FlyOut:seek(filter, timestamp)
    self.timestamp = timestamp
end

function FlyOut:apply(filter)
    self.count = #filter.label.chars
	local lineinfo = filter.label:getLineInfo()
	local h = 1280
    local w = 720
	local time = self.duration - self.timestamp
	self.allrowthCount = {}
	self.idInRow = {}
